...
async function getData() {
  const res = await fetch('http://localhost:3001/getMenu')
...
export default async function homepage() {
  ...
  return (
    <main className="flex flex-col items-center bg-background">
      <div className="navbar p-4 bg-white w-full h-auto flex flex-row drop-shadow-xl">
  ...
  <div className="grid grid-cols-2 p-6 gap-6">
    {data.map((menu, index) => (
      <a href="/foodoption" className="card indicator w-auto bg-white shadow-xl">
        <img className="h-32 w-full object-cover rounded-t-2xl" src={menu.menu_picture} alt={menu.menu_name} />
        {/* {index === 0 && (
          <span className="indicator-item badge bg-coral w-6 h-6 text-white font-extrabold mr-4 mt-4">
            1
          </span>
        )} */}
        <div className="card px-2 py-2 flex-row justify-between text-center items-center">
          ...
          <div className="card-actions justify-end">
            <button className="btn btn-xs btn-success bg-price text-white">
              ฿ {menu.price}
            </button>
          </div>
        </div>
      ...
      {/* <div className="sticky bottom-0 w-full bg-white mt-auto z-50">
        ...
        <div className="flex flex-col px-6 py-4 text-center items-center">
          ...
          >
            <button className="btn bg-coral w-full text-white flex items-center justify-center">
              ...
              <p className="font-bold ml-2 text-center flex-1">
                สั่งอาหาร - ฿60
              </p>
            </button>
          </a>
